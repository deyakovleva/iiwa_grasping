...
/*
bool GraspDetectionServer::detectGraspsPose(gpd_ros::detect_grasps_poses::Request &req, gpd_ros::detect_grasps_poses::Response &res)
{
  ROS_INFO("Received service request (poses)...");
  ...
  std::vector<std::unique_ptr<gpd::candidate::Hand>> poses = grasp_detector_->detectGrasps(*cloud_camera_);
  ...
  if (poses.size() > 0)
  {
    ...
    if (use_rviz_)
    {
      rviz_plotter_->drawGrasps(poses, frame_);
    }

    // Publish the detected grasps.
    gpd_ros::GraspConfigListPose selected_grasps_poses_msg = GraspMessages::createGraspListPoseMsg(poses, cloud_camera_header_);
    res.grasp_configs_poses = selected_grasps_poses_msg;

    ROS_INFO_STREAM("Detected " << selected_grasps_poses_msg.poses.size() << " highest-scoring grasps.");
    return true;
  }
...
//  ros::ServiceServer service = node.advertiseService("detect_grasps_poses", &GraspDetectionServer::detectGraspsPose,
//                                                     &grasp_detection_server);
